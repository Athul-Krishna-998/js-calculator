...
function commavalue(word)
{
  const wordno=word.toString();
  integer=parseFloat(wordno.split('.')[0]);
  decimal=wordno.split('.')[1];
  let intdisplay='';
  if(isNaN(integer))
  {
      intdisplay='';
...
{
    output.innerText=commavalue(currentnum);
    if(currentoperation!==null)
    {
    input.innerText=`${commavalue(oldnum)} ${currentoperation}`;
    }
}